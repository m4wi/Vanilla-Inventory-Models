import { PertCpm } from "../lib/cpm-pert.js";

const graph = new PertCpm()
graph.init();
...
function readInputTable() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const predecessors = cells[2].getElementsByTagName('input')[0].value;
    const optimisticTime = cells[3].getElementsByTagName('input')[0].value;
    const mostProbableTime = cells[4].getElementsByTagName('input')[0].value;
    const pessimisticTime = cells[5].getElementsByTagName('input')[0].value;
    // Agregar los datos a un objeto
    data.push([
      name , // Generar letra A, B, C, ...
      predecessors,
      parseInt(optimisticTime),
      parseInt(mostProbableTime),
      parseInt(pessimisticTime)
    ]);
  }
  return data;
}
...
function generateOutputTable (data) {
  ...
  data.forEach( dataRow => {
    ...
    <tr>
        ...
        <td>${ dataRow.optimisticTime }</td>
        <td>${ dataRow.mostProbableTime }</td>
        <td>${ dataRow.pessimisticTime }</td>
        <td>${ dataRow.time }</td>
        <td>${ dataRow.variance }</td>
        <td>${ dataRow.slack }</td>
